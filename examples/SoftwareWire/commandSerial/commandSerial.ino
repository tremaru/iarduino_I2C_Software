...
uint8_t fncFind    (                       int* m                   ){ uint8_t j=0; for(uint8_t i=1; i<128; i++){ sWire.beginTransmission(i); if( sWire.endTransmission()==0 ){ m[j++]=i; } } return j; }
uint8_t fncRead    ( uint8_t a, uint8_t r, int* m, uint8_t n        ){ sWire.beginTransmission(a); sWire.write(r); uint8_t i = sWire.endTransmission(false); if(i){ return i; } i = sWire.requestFrom(a, n); if(i==0){ for(i=0; i<n; i++){m[i]=0;} return 6; } if(i!=n){ for(i=0; i<n; i++){m[i]=0;} return 7; } i=0; while( sWire.available() ){ m[i++]=sWire.read(); } return 0; }
uint8_t fncRead    ( uint8_t a,            int* m, uint8_t n        ){                                             uint8_t i =                                                      sWire.requestFrom(a, n); if(i==0){ for(i=0; i<n; i++){m[i]=0;} return 6; } if(i!=n){ for(i=0; i<n; i++){m[i]=0;} return 7; } i=0; while( sWire.available() ){ m[i++]=sWire.read(); } return 0; }
uint8_t fncWrite   ( uint8_t a, uint8_t r, int* m, uint8_t n        ){ sWire.beginTransmission(a); sWire.write(r); for(uint8_t i=0; i<n; i++){ sWire.write(m[i]); } return sWire.endTransmission(); }
uint8_t fncWrite   ( uint8_t a,            int* m, uint8_t n        ){ sWire.beginTransmission(a);                 for(uint8_t i=0; i<n; i++){ sWire.write(m[i]); } return sWire.endTransmission(); }
